...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!category) return {};
  return { title: `${category.name} — NEAR Blog` };
}
  ...
  return (
    <div className="bg-white text-black">
      <section className="bg-gradient-to-br from-emerald-900 via-teal-800 to-green-900 py-20 px-6 text-center">
        <p className="text-emerald-300 text-xs tracking-widest uppercase font-mono mb-3">Category</p>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        {posts.length === 0 ? <p className="text-center text-gray-400 py-24">No posts yet.</p> : (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-4 mt-16">
                {page > 1 && <Link href={`/blog/category/${slug}?page=${page - 1}`} className="px-5 py-2 border border-gray-200 rounded-xl text-sm font-medium hover:bg-gray-50">← Previous</Link>}
                <span className="text-sm text-gray-400">Page {page} of {totalPages}</span>
                {page < totalPages && <Link href={`/blog/category/${slug}?page=${page + 1}`} className="px-5 py-2 border border-gray-200 rounded-xl text-sm font-medium hover:bg-gray-50">Next →</Link>}
              </div>
            )}
          </>
        )}
