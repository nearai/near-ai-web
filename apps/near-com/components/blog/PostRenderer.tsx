...
interface PostRendererProps {
  post: {
    ...
    coverImage?: string | null; heroBgColor?: string | null; heroBgImage?: string | null;
    content: any; publishedAt?: Date | null; updatedAt: Date;
    author: { name: string };
  ...
  return (
    <>
      <div style={heroStyle} className="relative">
        {post.heroBgImage && <div className="absolute inset-0 bg-black/50" aria-hidden="true" />}
        <div className="relative z-10 max-w-4xl mx-auto px-6 pt-16 pb-12">
          {layout === "public" ? (
            <div className={`text-sm ${textColorClass} opacity-70 mb-6 flex items-center gap-1`}>
              ...
              {post.categories.length > 0 && (
                ...
                <Link href={`/blog/category/${post.categories[0].slug}`} className="hover:opacity-100 transition">{post.categories[0].name}</Link></>
              )}
            </div>
          ) : (
          ...
          <p className={`text-sm ${textColorClass} opacity-70 mb-8`}>
            ...
            {layout === "admin" ? " (last updated)" : ` · ${readTime}`} · {post.author.name}
          </p>
          {post.coverImage && <img src={post.coverImage} alt={post.title} className="w-full rounded-xl object-cover" />}
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="prose prose-lg prose-emerald max-w-none text-gray-900">
